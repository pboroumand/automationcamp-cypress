let locators= require ("./locators")
export class loginPage {
    openLoginPage (){
        return cy.visit('https://opensource-demo.orangehrmlive.com')
    }
    enterUserName (username){
        return cy.get (locators.LOGINPAGE.USERNAME_INPUT).type(username)
    }
    enterPassword (password){
        return cy.get(locators.LOGINPAGE.PASSWORD_INPUT).type(password)
    }
    clickOnLoginButton (){
        return cy.get(locators.LOGINPAGE.LOGIN_BUTTON).click()
    }
    verifyInvalidCredMessage (){
        return cy.contains('Invalid credentials').should('be.visible')
    }
    loginWithAdminUser (){
        this.openLoginPage()
        this.enterUserName('Admin')
        this.enterPassword('admin123')
        this.clickOnLoginButton()
        return this
    }
}